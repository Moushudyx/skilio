...
7. 同步检测到的智能体/IDE 对应规则文件中的 `<skilio>...</skilio>`：
   - 若未检测到任何智能体/IDE，则回退写入根目录 `AGENTS.md` 并输出警告。
   - 目标规则文件不存在时创建。
   - 目标规则文件为符号链接时跳过并记录 warning/debug 日志。
   - 不存在 `<skilio></skilio>` 时追加引导文案与空标签。
   - 用本次 `node_modules` 扫描索引整体替换 `<skilio>...</skilio>`。
8. 输出扫描进度与结果摘要。
9. 遇到同名冲突或不可继续错误，记录 debug 日志并退出。
...
1. 读取 `installSources` 并解析来源。
2. 若未指定技能：卸载整个来源并删除已安装技能。
3. 若指定技能：仅卸载匹配的已安装技能并更新记录。
4. 更新 `skillDisabled` 并同步目标智能体/IDE。
