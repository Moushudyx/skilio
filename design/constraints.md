...
- 解析失败时：
  ...
  - 本次扫描跳过该技能。
- 若检测到 `SKILL.md` 存在但无效，或技能名与目录名不一致：
  - 需要在控制台打印提示信息。
  - 若未发现 `SKILL.md`，不打印控制台错误。
...
- 卸载仅针对 `installSources` 中记录的来源。
- 卸载来源中的部分技能时：
  - 必须从 `installed` 列表中移除。
  - `mode=all` 需将技能加入 `exclude`，防止更新时重新安装。
  - `mode=only` 需将技能加入 `exclude`，并移除等值 `include` 项（非通配符）。
- 若指定技能未命中已安装列表，必须提示并保持配置不变。
...
- 安装：记录来源、模式与技能列表。
  - 模式 `all`：默认安装全部技能，允许配置 `exclude` 排除。
  - 模式 `only`：仅安装 `include` 匹配的技能（支持通配符）。
